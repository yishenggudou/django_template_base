...
UserView = Backbone.View.extend({
    ...
    initialize:function(){
        ...
        var that = this;
        this.template=$('#tmpl_${NAME}s').html();
        console.log(['------------------'])
        ...
        this._events = {};
        //事件绑定为当前小局部的元素
        $('tr[cid='+this.model.cid+'] td.name').live("click",this.open${NAME});
        $('tr[cid='+this.model.cid+'] td.save').live('click',this.save);
        //this._events[('click tr[cid='+this.model.cid+'] td.name')] = 'open${NAME}';
    ...
    render :function(){
        ...
        console.log(['Start render model ${NAME}',this.template,to_render])
...
UserCollection  = Backbone.Collection.extend({
    ...
    resetsets:function(obj){
        console.log(["${NAME} Sets has add ",obj]);
        $('#${NAME}s tr').remove();
        this.each(function(item){
            console.log(["render model",item]);
            var ${NAME}v = new UserView({model:item});
            ${NAME}v.render();
        })
    },
    addobj:function(){
